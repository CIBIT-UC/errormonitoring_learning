    ...
    for j = runs
        ...
        if exist(strcat(spm_folder, '/SPM.mat'))
            ...
            for t = trials
                ...
                mixed_model{idx,1} = data(i).name;        % Participant
                mixed_model{idx,2} = str2num(data_func(j).name(end));   % Run
                ...
                if sum(ismember(progreen, protocol{ntrial,5})) == 1
                    mixed_model{idx,3} = "Pro_green";        % Instruction
                elseif sum(ismember(prored, protocol{ntrial,5})) == 1
                    mixed_model{idx,3} = "Pro_red";        % Instruction
                elseif sum(ismember(antigreen, protocol{ntrial,5})) == 1
                    mixed_model{idx,3} = "Anti_green";        % Instruction
                elseif sum(ismember(antired, protocol{ntrial,5})) == 1
                    mixed_model{idx,3} = "Anti_red";        % Instruction
                end
                
                saccade = [5:8,13:16];
                keypress = [1:4,9:12];
                if sum(ismember(saccade, protocol{ntrial,5})) == 1
                    mixed_model{idx,4} = "Saccade";        % Action
                elseif sum(ismember(keypress, protocol{ntrial,5})) == 1
                    mixed_model{idx,4} = "Keypress";        % Action
                end
                ...
                mixed_model{idx,6} = b_dACC;
                mixed_model{idx,7} = b_dACC/b_base_dACC*100; % Signal change
                
                % Beta AI
                b_AI = mean(spm_summarise(beta_file, AI_voi));
                mixed_model{idx,8} = b_AI;
                mixed_model{idx,9} = b_AI/b_base_AI*100; % Signal change

                % Beta putamen
                b_put = mean(spm_summarise(beta_file, put_voi));
                mixed_model{idx,10} = b_put;
                mixed_model{idx,11} = b_put/b_base_put*100;  % Signal change
...
mixed_model_table = cell2table(mixed_model, 'VariableNames', ...
    {'Participant'; 'Run'; 'Instruction'; 'Action'; 'Performance'; ...
    'b_dACC'; 'sc_dACC'; 'b_AI'; 'sc_AI'; 'b_put'; 'sc_put'});
